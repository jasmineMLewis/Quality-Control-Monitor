...
Public Class CreateLeasing
    ...
    Const REVIEW_TYPE_ID As Integer = 8
...
#Region "For Error Checkboxes (Documents)"
    ...
    Const DOCUMENT_HUD_TENANCY_ADDEDUM As Integer = 34
    ...
    Const DOCUMENT_INITIAL_UA_CALCULATION_WORKSHEET_ELITE As Integer = 44
...
#End Region
    ...
    Public Function AssignSeededReviewDocuments(ByVal data As Dictionary(Of Integer, Boolean)) As Dictionary(Of Integer, Boolean)
        ...
        If Not Request.Form("documentHudTenancyAddendum") Is Nothing Or Not Request.Form("documentHudTenancyAddendum") = "" Then
            data(DOCUMENT_HUD_TENANCY_ADDEDUM) = True
        End If
        ...
        If Not Request.Form("documentInitialUaCalculationWorksheetElite") Is Nothing Or Not Request.Form("documentInitialUaCalculationWorksheetElite") = "" Then
            data(DOCUMENT_INITIAL_UA_CALCULATION_WORKSHEET_ELITE) = True
        End If
    ...
    Public Sub DisplayDropDownlistCaseManager(ByVal fileID As Integer)
        ...
        If Not IsPostBack Then
            If caseManagerID <> 0 Then
                ...
                CaseManagerInitialRentLetter.Items.FindByValue(caseManagerID).Selected = True

                'Initial Hud Form 50058
                CaseManagerInitialHudForm50058.DataBind()
                CaseManagerInitialHudForm50058.Items.FindByValue(caseManagerID).Selected = True

                'Initial Rent Calculation Sheet
                CaseManagerInitialRentCalculationSheet.DataBind()
                CaseManagerInitialRentCalculationSheet.Items.FindByValue(caseManagerID).Selected = True

                'Initial Ua Calculation Worksheet - Elite
                CaseManagerInitialUaCalculationWorksheetElite.DataBind()
                CaseManagerInitialUaCalculationWorksheetElite.Items.FindByValue(caseManagerID).Selected = True

                'Hap Contract (Initial Unit)
                CaseManagerHapContractInitialUnit.DataBind()
                CaseManagerHapContractInitialUnit.Items.FindByValue(caseManagerID).Selected = True

                'Hud Tenancy Addendum (Initial Unit)
                CaseManagerHudTenancyAddendumInitialUnit.DataBind()
                CaseManagerHudTenancyAddendumInitialUnit.Items.FindByValue(caseManagerID).Selected = True

                'Lease (Initial Unit)
                CaseManagerLeaseInitialUnit.DataBind()
                CaseManagerLeaseInitialUnit.Items.FindByValue(caseManagerID).Selected = True

                'Rfta (Initial Unit)
                CaseManagerRftaInitialUnit.DataBind()
                CaseManagerRftaInitialUnit.Items.FindByValue(caseManagerID).Selected = True

                'Utility Allowance Checklist (Initial Unit)
                CaseManagerUtilityAllowanceChecklistInitialUnit.DataBind()
                CaseManagerUtilityAllowanceChecklistInitialUnit.Items.FindByValue(caseManagerID).Selected = True
